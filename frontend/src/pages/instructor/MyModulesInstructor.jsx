...
const MyCoursesInstructor = () => {
    ...
    const [contentImage, setContentImage] = useState(null); // File object for image upload
    const [contentVideoUrl, setContentVideoUrl] = useState(""); // String for video URL/link
    ...
    // 🚀 CORRECTED LOGIC FOR IMAGE FILE + VIDEO URL SUBMISSION
    const handleAddContent = async (e) => {
        ...
        // Use 'videopath' key to send the URL link, matching the controller's expectation
        formData.append("videopath", contentVideoUrl || ""); 

        // Append image file only if available. Key must match Multer's 'single("image")'
        if (contentImage) {
        ...
        try {
            await axios.post(
                ...
                formData, 
                // CRITICAL: Must include header for FormData
                { headers: { "Content-Type": "multipart/form-data" } } 
            );
            toast.success("Content added successfully");
        } catch (err) {
            toast.error("Failed to add content. Check server logs.");
            console.error("Content add error:", err.response?.data || err.message);
        }
    ...
    return (
        <div className="container py-4 page-content">
            ...
            {/* Add Content Modal (Image/Video Inputs) */}
            ...
            {/* Enroll Trainee Modal (Unchanged) */}
